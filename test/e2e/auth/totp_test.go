...
func TestTOTP_PasswordStepDoesNotRecordFinalLoginSideEffects(t *testing.T) {
	...
	beforeUserLastLogin := totpScalarString(t, "SELECT COALESCE(last_login_at::text, '') FROM users WHERE id = $1", userID)
	beforeIdentityLastLogin := totpScalarString(t, "SELECT COALESCE(last_login_at::text, '') FROM user_identities WHERE user_id = $1 AND provider = 'password'", userID)
	beforeSuccessAudits := totpScalarInt(t, "SELECT COUNT(*) FROM audit_logs WHERE user_id = $1 AND action = 'auth.login.success'", userID)
	...
	afterUserLastLogin := totpScalarString(t, "SELECT COALESCE(last_login_at::text, '') FROM users WHERE id = $1", userID)
	afterIdentityLastLogin := totpScalarString(t, "SELECT COALESCE(last_login_at::text, '') FROM user_identities WHERE user_id = $1 AND provider = 'password'", userID)
	afterSuccessAudits := totpScalarInt(t, "SELECT COUNT(*) FROM audit_logs WHERE user_id = $1 AND action = 'auth.login.success'", userID)
...
func TestTOTP_SessionRevocationOnChangeWhenConfigured(t *testing.T) {
	...
	revoked := totpScalarInt(t, "SELECT COUNT(*) FROM auth_sessions WHERE user_id = (SELECT id FROM users WHERE email = $1) AND revoked_at IS NOT NULL", email)
	if revoked < 2 {
...
func TestTOTP_AuditEventsAreRecorded(t *testing.T) {
	...
	for _, action := range []string{
		...
		count := totpScalarInt(t, "SELECT COUNT(*) FROM audit_logs WHERE user_id = $1 AND action = $2", userID, action)
		if count < 1 {
...
func totpScalarString(t *testing.T, sql string, args ...any) string {
	t.Helper()
...
func totpScalarInt(t *testing.T, sql string, args ...any) int {
	t.Helper()
	value, err := g.DB().GetValue(context.Background(), sql, args...)
	if err != nil {
