  ...
  return (
    <div className="space-y-6">
      ...
      <Card>
        ...
        {tenants.isLoading ? <LoadingView label="加载组织..." /> : <div className="overflow-x-auto"><Table><thead><tr><Th>组织</Th><Th>状态</Th><Th>Plan</Th><Th>操作</Th></tr></thead><tbody>{tenants.data?.items.map((tenant) => <tr key={tenant.id}><Td><div className="font-bold text-ink">{tenant.name}</div><div className="text-xs text-subtle">{tenant.id}</div></Td><Td><Badge tone={tenant.status === 'active' ? 'green' : 'orange'}>{tenant.status}</Badge></Td><Td>{tenant.plan}</Td><Td><div className="flex flex-wrap gap-2"><Button size="sm" variant="secondary" leftIcon={<Shield className="size-3" />} onClick={() => actions.suspend.mutate(tenant.id)} disabled={!canManageTenants || tenant.status !== 'active'}>暂停</Button><Button size="sm" variant="secondary" onClick={() => actions.restore.mutate(tenant.id)} disabled={!canManageTenants || tenant.status === 'active'}>恢复</Button></div></Td></tr>)}</tbody></Table></div>}
      </Card>
    </div>
  )
}
