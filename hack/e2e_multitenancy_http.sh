...
b="$(bodyfile)"; s="$(http_request GET /api/v1/me "" "${b}" -H "X-User-ID: not-a-uuid")"; assert_status AUTH_BAD_USER 401 "${s}" "${b}"

create_payload="{\"name\":\"${tenant_name}\",\"slug\":\"${tenant_slug}\"}"
b="$(bodyfile)"; s="$(http_request POST /api/v1/tenants "${create_payload}" "${b}" -H "X-User-ID: ${owner_id}")"; assert_status TENANT_CREATE 200 "${s}" "${b}"
tenant_id="$(json_value "${b}" 'j["data"]["tenant"]["id"]')"
assert_json_equals TENANT_CREATE_SLUG "${b}" 'j["data"]["tenant"]["slug"]' "${tenant_slug}"

b="$(bodyfile)"; s="$(http_request GET /api/v1/me "" "${b}" -H "X-User-ID: ${owner_id}")"; assert_status ME_OWNER 200 "${s}" "${b}"; assert_json_equals ME_OWNER_ID "${b}" 'j["data"]["user"]["id"]' "${owner_id}"
b="$(bodyfile)"; s="$(http_request GET /api/v1/me/tenants "" "${b}" -H "X-User-ID: ${owner_id}")"; assert_status ME_TENANTS 200 "${s}" "${b}"

b="$(bodyfile)"; s="$(http_request GET /api/v1/tenant-context "" "${b}" -H "X-User-ID: ${owner_id}")"; assert_status TENANT_MISSING 400 "${s}" "${b}"
b="$(bodyfile)"; s="$(http_request GET /api/v1/tenant-context "" "${b}" -H "X-User-ID: ${outsider_id}" -H "X-Tenant-ID: ${tenant_id}")"; assert_status TENANT_OUTSIDER 403 "${s}" "${b}"
b="$(bodyfile)"; s="$(http_request GET /api/v1/tenant-context "" "${b}" -H "X-User-ID: ${owner_id}" -H "X-Tenant-ID: ../etc/passwd")"; assert_status TENANT_MALFORMED_SELECTOR 403 "${s}" "${b}"

b="$(bodyfile)"; s="$(http_request GET /api/v1/tenant-context "" "${b}" -H "X-User-ID: ${owner_id}" -H "X-Tenant-ID: ${tenant_id}")"; assert_status TENANT_CONTEXT_OWNER 200 "${s}" "${b}"; assert_json_equals TENANT_CONTEXT_ROLE "${b}" 'j["data"]["tenant_context"]["role"]' owner

add_member_payload="{\"user_id\":\"${viewer_id}\",\"role\":\"viewer\",\"status\":\"active\"}"
b="$(bodyfile)"; s="$(http_request POST "/api/v1/tenants/${tenant_id}/members" "${add_member_payload}" "${b}" -H "X-User-ID: ${owner_id}")"; assert_status MEMBER_ADD_VIEWER 200 "${s}" "${b}"; assert_json_equals MEMBER_ADD_ROLE "${b}" 'j["data"]["member"]["role"]' viewer

b="$(bodyfile)"; s="$(http_request GET /api/v1/tenant-context "" "${b}" -H "X-User-ID: ${viewer_id}" -H "X-Tenant-ID: ${tenant_id}")"; assert_status TENANT_CONTEXT_VIEWER 200 "${s}" "${b}"; assert_json_equals VIEWER_ROLE "${b}" 'j["data"]["tenant_context"]["role"]' viewer
...
b="$(bodyfile)"; s="$(http_request POST "/api/v1/tenants/${tenant_id}/api-keys" '{"name":"bad-scope","scopes":["unknown"]}' "${b}" -H "X-User-ID: ${owner_id}" -H "X-Tenant-ID: ${tenant_id}")"; assert_status APIKEY_BAD_SCOPE 400 "${s}" "${b}"
personal_key_payload="{\"name\":\"limited\",\"scopes\":[\"tenant:read\",\"member:read\"]}"
b="$(bodyfile)"; s="$(http_request POST "/api/v1/tenants/${tenant_id}/api-keys" "${personal_key_payload}" "${b}" -H "X-User-ID: ${owner_id}" -H "X-Tenant-ID: ${tenant_id}")"; assert_status APIKEY_CREATE_LIMITED 200 "${s}" "${b}"
raw_key="$(json_value "${b}" 'j["data"]["raw_key"]')"
api_key_id="$(json_value "${b}" 'j["data"]["api_key"]["id"]')"

b="$(bodyfile)"; s="$(http_request GET "/api/v1/tenants/${tenant_id}/api-keys" "" "${b}" -H "X-User-ID: ${owner_id}" -H "X-Tenant-ID: ${tenant_id}")"; assert_status APIKEY_LIST_TENANT 200 "${s}" "${b}"
...
b="$(bodyfile)"; s="$(http_request DELETE "/api/v1/tenants/${tenant_id}/api-keys/${api_key_id}" "" "${b}" -H "X-User-ID: ${owner_id}" -H "X-Tenant-ID: ${tenant_id}")"; assert_status APIKEY_REVOKE 200 "${s}" "${b}"
...
b="$(bodyfile)"; s="$(http_request POST "/api/v1/tenants/${tenant_id}/api-keys" '{bad-json' "${b}" -H "X-User-ID: ${owner_id}" -H "X-Tenant-ID: ${tenant_id}")"; assert_status MALFORMED_JSON 400 "${s}" "${b}"
