-- system_config: system-wide configuration with tenant-level override support.
-- tenant_id IS NULL = global default; non-NULL = tenant-specific override.
CREATE TABLE IF NOT EXISTS public.system_config (
    ...
    CONSTRAINT fk_system_config_tenant FOREIGN KEY (tenant_id)
        REFERENCES public.tenants(id) ON DELETE CASCADE
);

CREATE INDEX IF NOT EXISTS idx_system_config_tenant
    ON public.system_config(tenant_id) WHERE tenant_id IS NOT NULL;
CREATE INDEX IF NOT EXISTS idx_system_config_key
    ON public.system_config(key) WHERE tenant_id IS NULL;

COMMENT ON TABLE  public.system_config IS 'System configuration with tenant-level override support';
...
COMMENT ON COLUMN public.system_config.tenant_id IS 'Tenant ID; NULL = global default';
COMMENT ON COLUMN public.system_config.is_encrypted IS 'Whether value is AES-256-GCM encrypted';

-- ============================================================================
-- Seed data: global defaults migrated from config.yaml
-- Secret values must be set via admin API or direct INSERT after deployment.
-- ============================================================================
